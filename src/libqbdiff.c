...
#define QBDIFF_MAGIC_BIG "ABDB1"
#define QBDIFF_MAGIC_FULL "ABDF1"
...
// BLAKE3 checksum wrapper.
static void blake3_cksum(const uint8_t * data, int64_t size, uint8_t cksum[BLAKE3_OUT_LEN]) {
    blake3_hasher hasher;
    blake3_hasher_init(&hasher);
    blake3_hasher_update(&hasher, data, size);
    memset(cksum, 0, BLAKE3_OUT_LEN);
    blake3_hasher_finalize(&hasher, cksum, BLAKE3_OUT_LEN);
}
...
LIBQDIFF_PUBLIC_API int qbdiff_compute(const uint8_t * RESTRICT old, const uint8_t * RESTRICT new, size_t old_size,
    ...
    int err_code = 0;
    uint8_t cksum[BLAKE3_OUT_LEN];
    blake3_cksum(new, new_size, cksum);
    ...
    if (old_size < 256 || new_size < 256) {
        ...
        if (fwrite(cksum, 1, BLAKE3_OUT_LEN, diff_file) != BLAKE3_OUT_LEN) return QBERR_IOERR;
...
#define sfwrite(ptr, size, nmemb, stream)            \
    ...
    if (ml.cblen + ml.dblen + ml.eblen > 0.9 * new_size) {
        ...
        if (fwrite(cksum, 1, BLAKE3_OUT_LEN, diff_file) != BLAKE3_OUT_LEN) goto err;
        ...
        sfwrite(cksum, 1, BLAKE3_OUT_LEN, diff_file);
        uint8_t buf[8];
        wi64(old_size, buf);
...
LIBQDIFF_PUBLIC_API int qbdiff_patch(const uint8_t * RESTRICT old, const uint8_t * RESTRICT patch, size_t old_len,
    ...
    if (!memcmp(patch, QBDIFF_MAGIC_FULL, 5)) {
        ...
        uint8_t new_cksum[BLAKE3_OUT_LEN];
        int64_t uncompressed_size = ri64(patch + 69);
        uint8_t * uncompressed;
        int result = decompress(patch + 77, patch_len - 77, &uncompressed, uncompressed_size);
        if (result != QBERR_OK) return result;
        blake3_cksum(uncompressed, uncompressed_size, new_cksum);
        if (memcmp(patch + 5, new_cksum, BLAKE3_OUT_LEN)) return QBERR_BADCKSUM;
        ...
        uint8_t new_cksum[BLAKE3_OUT_LEN];
        blake3_cksum(new_data, new_size, new_cksum);
        if (memcmp(patch + 5, new_cksum, BLAKE3_OUT_LEN)) {
            errn = QBERR_BADCKSUM;
            goto err;
        }
